! function(n) {
	...
	function initArticle(type, index) {
		//修改这里load里的内容改为http .load("article/article"+type+".html"......
		$("#article" + index).load("article/article"+type+".html",function (responseTxt, statusTxt, xhr) {
	...
	function initPage(){
		...
		initAnswer(beginArticle, 1);
		initAnswer(String(Number(beginArticle) + 1), 2);
		initAnswer(String(Number(beginArticle) + 2), 3);
		initArticle(beginArticle, 1);
		initArticle(String(Number(beginArticle) + 1), 2);
		initArticle(String(Number(beginArticle) + 2), 3);
	}
